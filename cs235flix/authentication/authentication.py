...
def register():
    ...
    if form.validate_on_submit():
        ...
        try:
            auth_services.add_user(form.username.data, form.password.data, repo.repo_instance)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        try:
            user = auth_services.get_user(form.username.data, repo.repo_instance)

            # Authenticate user.
            auth_services.authenticate_user(user['username'], form.password.data, repo.repo_instance)
...
class PasswordValid:
    def __init__(self, message=None):
        if not message:
            message = u'Your password must at least 8 characters, and contain an upper case letter, a ' \
